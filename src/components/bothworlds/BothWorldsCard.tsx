...
export default function BothWorldsCard({image,bgColor,logoPos,label,heading,text,button} : {
    image : string,
    bgColor : string,
    logoPos : string,
    label : string,
    ...
    useGSAP(() => {
        ...
        timeline.fromTo(logoRef.current, {y: -500}, {y: -100, ease: "linear"})
    ...
    return (
        <div ref={cardRef} className="w-full bg-white flex flex-col">

            {/* image + logo */}
            <div className={`relative w-full min-h-[47vw] md:min-h-[20vw] lg:min-h-[23vw] overflow-hidden ${bgColor === "purple" ? "bg-purple" : "bg-primary"}`}>
                <img src={`/BothWorldsCards/${image}`} className="z-1 absolute w-[90%] left-[50%] -translate-x-[50%] -bottom-[12%] object-cover" />
                <svg ref={logoRef} className={`absolute w-[790px] h-[1100px] ${logoPos === "left" ? "left-0 top-0 -translate-y-[470px] -translate-x-[100px] -rotate-[25deg] fill-purple" : "left-0 top-0 -translate-y-[470px] -translate-x-[100px] -rotate-[154deg] fill-primary"}`} viewBox="0 0 23 30">
            ...
            {/* text + btn */}
            <div className="flex flex-col h-full justify-around w-full p-[24px] md:p-[32px] lg:p-[40px] items-start">
                <div>
                    <p className="font-abc-diatype-bold text-sm-desktop mb-[8px] text-sublabel">{label}</p>
                    <p className="font-rale-grotesk-base-medium text-h4-mobile leading-h4-mobile md:text-h4-tablet md:leading-h4-tablet lg:text-h4-desktop lg:leading-h4-desktop mb-[8px] text-heading">{heading}</p>
                    <p className="text-md-mobile leading-md-mobile md:text-md-tablet md:leading-md-tablet lg:text-md-desktop lg:leading-md-desktop font-abc-diatype-regular text-subtext mb-[32px]">{text}</p>
                </div>
                <div className="mt-auto w-full">
                    <Button color="black" type="primary" arrow={true}>Learn More</Button>
                </div>
            </div>
