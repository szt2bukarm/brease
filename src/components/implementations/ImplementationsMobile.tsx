...
import ImplementationsDropdownSection from "./ImplementationnDropdownSection";
import { useGSAP } from "@gsap/react";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import gsap from "gsap";
import AIPromptCard from "../aipromptcard/AIPromptCard";
gsap.registerPlugin(ScrollTrigger)

export default function ImplementationsMobile({sections, openedSectionTitle, handleSelection}: any) {
    const cardRef = useRef<HTMLDivElement>(null)
    const logoRef = useRef<SVGSVGElement>(null)


    // parallax logo scroll
    useGSAP(() => {
        const timeline = gsap.timeline()

        timeline.fromTo(logoRef.current, {y: -200}, {y: 200, ease:"linear"})
    ...
    return (
        <div className="flex flex-col w-full px-[24px] pb-[80px] lg:pb-[120px] bg-background">
            ...
            <Heading>Powerful feature that set us apart</Heading>

            <p className="font-rale-grotesk-base-regular text-heading text-h5-mobile leading-h5-desktop md:text-h5-tablet lg:text-h5-desktop mt-[40px] pb-[40px] pl-0 sm:pl-[40px] sm:border-l-6 sm:border-purple"><b>AI Assistant That Does, Not Just Suggests</b> Forget simple content prompts — Brease’s AI takes action.Create entire pages, update sections, or restructure layouts just by asking. It understands your design rules and executes updates without you digging through content trees or nested settings.</p>

            <div ref={cardRef} className="relative w-full h-[432px] flex items-center justify-center bg-primary p-[40px] overflow-hidden mb-[32px]">
                <svg ref={logoRef} className={`absolute w-[790px] h-[1100px] fill-purple z-1 -rotate-[25deg] right-0 -top-[35%]`} viewBox="0 0 23 30">
                ...
                {/* <img src="/ImplementationsCards/1.png" className="z-2 w-[85%] max-w-[432px] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-90%] shadow-[0_56px_112px_rgba(62,52,69,0.18)]"/>
                <img src="/ImplementationsCards/2.png" className="z-1 w-[80%] max-w-[400px] absolute top-[50%] left-[50%] translate-x-[-50%] -translate-y-[56%] [clip-path:polygon(0_50%,100%_50%,100%_100%,0%_100%)]" /> */}
            </div>

            <div className="flex flex-col gap-[24px]">
            <div className="flex flex-col md:pl-[48px] lg:pl-[80px] mt-auto">
                {sections.map(({ title, content }, index) => (
                    <ImplementationsDropdownSection
                        key={title}
                        border={index !== sections.length - 1}
                        selected={openedSectionTitle === title}
                        title={title}
