...
export default function BreaseAI() {
    ...
    useGSAP(() => {
        ...
        timeline.fromTo(logoRef.current, {y: -100}, {y: 300, ease: "linear"})
    ...
    return (
        <div ref={cardRef} className="relative w-full h-full px-[24px] md:px-[90px] lg:px-[11vw] pb-[80px] lg:pb-[120px] bg-background">

            {/* wrapper */}
            <div className="relative flex flex-col md:flex-row justify-between z-1 p-[24px] md:p-[40px] bg-white md:min-h-[552px] shadow-[0_56px_112px_-20px_rgba(62,52,69,0.18)] overflow-hidden">

                {/* left col */}
                <div className="relative flex flex-col items-start w-full md:w-[35%] md:min-h-[432px] z-1">
                    ...
                    <Button color="black" type="primary" arrow={true}>More about AI</Button>
                </div>

                {/* bg logo */}
                <svg ref={logoRef} className={`absolute w-[790px] h-[1100px] md:w-[990px] md:h-[1300px] fill-purple rotate-[25deg] scale-x-[-1] max-md:-left-[270px] -bottom-[550px] md:-right-60 lg:-right-30 md:-top-[70%] translate-x-15`} viewBox="0 0 23 30">
                ...
                {/* image */}
                <div className=" sm:translate-y-0 md:translate-y-5 lg:translate-y-15 md:-translate-x-15">
                <AIPromptCard scaleVariant={2} emptyInput={false}/>
                </div>
                {/* <img src="/ImplementationsCards/2.png" className="w-full relative md:absolute z-2 md:right-[10%] md:translate-y-1/2 md:bottom-[50%] md:max-w-[440px] lg:max-w-none lg:translate-y-0 lg:-bottom-[90px] md:w-[42%] lg:w-[520px] shadow-[0_40px_80px_-16px_rgba(62,52,69,0.16)]"/> */}
            </div>

            <div className="absolute bottom-0 left-0 w-full h-[200px] bg-white"></div>
        </div>
    )
}